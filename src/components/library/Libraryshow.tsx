...
const Libraryshow = () => {
  ...
  return (
    <>
      <Grid item xs={12}>
        {data.mode === 0 && (
          <Box className={"fs-40-bold white"}>{t("lib-1_your-games")}</Box>
        )}
        {data.mode === 1 && (
          <Box className={"fs-40-bold white"}>{t("lib-2_wishlist")}</Box>
        )}
        {data.mode === 2 && (
          <Box className={"fs-40-bold white"}>{t("lib-3_download")}</Box>
        )}
      </Grid>
